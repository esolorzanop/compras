<?php
		...
		$con=conectar();
			...
			$nom_item = strtoupper(isset($_REQUEST["nom_item"]) ? $_REQUEST["nom_item"]:NULL);			
			$cod_cate = strtoupper(isset($_REQUEST["cod_item"]) ? $_REQUEST["cod_cate"]:NULL);			
				...
				var varRut = $('#nomitem').val();
						...
						if (tecla==13) {
							if(varRut == ''){
								...
								if(varRut.split("_")[2]){
									$('#nomitem').val(varRut.split("_")[2]);
									$('#nom_item').val(varRut.split("_")[2]);						
									$('#cod_item').val(varRut.split("_")[0]);
									$('#nom_cate').val(varRut.split("_")[1]);
								}
							}
							send('consulta');
						}
...
<h2 class="title">BUSQUEDA HISTORIAL DE ITEMS ADQUIRIDOS -  <?php echo $_SESSION['user']; ?></h2>
<br />
<div align="center">
...
<input name="actualizar" type="button" class="botones" id="actualizar" value="Recargar Página" onclick="location.href='bushisto.php'"/>
</div>          
<br />
<?php 	if($cod_item==""){ ?>
...
<tr>
	<td height="25" colspan="9" bgcolor="#444444" ><div align="center" class="Estilo56">BUSQUEDA POR ITEMS COTIZADOS</div></td>   
</tr>
<tr><td bgcolor='#FFFFFF' colspan="9">
      <div align="center"><span class="Estilo53"><span class="Estilo59">Atenci&oacute;n:</span><span class="Estilo45"> <strong>Para realizar la busqueda, debe ingresar el nombre del item en la caja de texto y esperar los resultados se vayan mostrando.</strong></span></span></div>
</td></tr>
<tr height="25">
    <td width="54" height="25"  colspan="3" bgcolor="#626262" class="date1"><div align="center" class="Estilo55">Item a buscar:</div>
    </td>
	<td bgcolor='#FFFFFF' class="date" colspan="6">
   &nbsp; &nbsp;<input name="nomitem" type="text" id="nomitem" size="35" autofocus/> <br/>
...
<?php
$sql_dtr="SELECT * FROM (
	SELECT ROWNUM AS FILAS, CO.CO_FECHA,DC.COD_ITEM,IT.IT_NOMBRE,CO.COD_COTIZACION,CO.COD_PEDIDO,CO.CODI_INVE_PROV,VP.RAZO_SOCI_INVE_PROV,DC.DC_VUNITARIO,DC.DC_CANTIDAD,DC.DC_VALXCANTIDAD,DC.DC_VALDESCUENTO,CO.CO_APROBADA, COD_VEHICULO FROM DET_COTIZACION DC,COTIZACION CO,ITEM IT,VIEW_PROVEEDORES VP WHERE CO.COD_COTIZACION(+) = DC.COD_COTIZACION AND DC.COD_ITEM = '".$cod_item."' AND IT.COD_ITEM(+) = DC.COD_ITEM AND VP.CODI_INVE_PROV(+) = CO.CODI_INVE_PROV ORDER BY  CO_FECHA DESC) WHERE ROWNUM < 22";
		...
		if ($cont == 0)
			{
				...
				if($row_dtr['DC_VUNITARIO'] <> $dc_uni){										
					...
					$valores[$cont1]['PEDID'] = $row_dtr['COD_PEDIDO'];		
...
<?php  
}else{  ?> <div colspan="5" bgcolor='#fff' class='Estilo58' align='center'>* Su busqueda no obtuvo resultados, intente recargar la página y  pruebe con un item distinto. </div>
